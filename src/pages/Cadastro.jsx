...
//           {/* CORREÇÃO: onChange formata e guarda já formatado — evita bug de cursor */}
//           <F name="telefone" label="Telefone" required>
//             <input
//               className={`form-input${erros.telefone ? " error" : ""}`}
//               value={form.telefone}
...
//           {/* CORREÇÃO: removido max={} — data livre para frente e trás */}
//           <F name="data_inicio" label="Data início do tratamento" required>
//             <input
//               className={`form-input${erros.data_inicio ? " error" : ""}`}
//               type="date"
...
// ✅ F está FORA do componente Cadastro.
// Recebe `erros` como prop em vez de capturá-lo por closure.
// Assim o React reconhece o mesmo componente entre renders e
// nunca desmonta/remonta os inputs — o foco é preservado.
const F = ({ name, label, children, required, erros }) => (
  <div className={`form-group${name === "nome" || name === "observacao" || name === "medicamento" ? " full" : ""}`}>
    <label className="form-label">{label}{required && " *"}</label>
    {children}
    {erros[name] && <span className="form-input-error">⚠ {erros[name]}</span>}
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <div className="form-grid">
          <F name="nome" label="Nome completo" required erros={erros}>
            <input
              className={`form-input${erros.nome ? " error" : ""}`}
              value={form.nome}
              onChange={e => set("nome", e.target.value)}
          ...
          <F name="telefone" label="Telefone" required erros={erros}>
            <input
              className={`form-input${erros.telefone ? " error" : ""}`}
              value={form.telefone}
              onChange={e => set("telefone", formatarTelefone(e.target.value))}
          ...
          <F name="cpf" label="CPF" required erros={erros}>
            <input
              className={`form-input${erros.cpf ? " error" : ""}`}
              value={form.cpf}
              onChange={e => set("cpf", formatarCPF(e.target.value))}
          ...
          <F name="total_ml" label="Quantidade total (ml)" required erros={erros}>
            <input
              className={`form-input${erros.total_ml ? " error" : ""}`}
              type="number" min="0.1" step="0.5" max="9999"
              value={form.total_ml}
          ...
          <F name="data_inicio" label="Data início do tratamento" required erros={erros}>
            <input
              className={`form-input${erros.data_inicio ? " error" : ""}`}
              type="date"
              value={form.data_inicio}
          ...
          <F name="medicamento" label="Medicamento" required erros={erros}>
            <select
              className="form-input"
              value={form.medicamento}
              onChange={e => set("medicamento", e.target.value)}
          ...
          <F name="observacao" label="Observações / Informações adicionais" erros={erros}>
            <textarea
              className="form-input"
              value={form.observacao}
              onChange={e => set("observacao", e.target.value)}
